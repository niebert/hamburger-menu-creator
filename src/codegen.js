...
function writeConvertJSON(srcPath, savPath, pJSON) {
	// console.log('CALL: writeConvertJSON() "'+srcPath+'","'+savPath+'",pJSON)-Call');
	// console.log('CALL: writeConvertJSON() replace in File "'+srcPath+'"');
	fs.readFile(srcPath, 'utf8', function(err, contents) {
    var vContent = processJSON(contents,pJSON);
    if (vContent) {
      fs.writeFile (savPath, vContent, function(err) {
          if (err) throw err;
          console.log('writeConvertJSON("'+savPath+'")-Call DONE');
      });
    } else {
